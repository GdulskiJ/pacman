...
public class Planszagenerator {
    ...
    public Planszagenerator(int dlugosc, int szeroskosc) {
        ...
        do {
            ...
            tworzenievertex();
            tworzenieedges();
            List<Vertex> ph = Lokomotywa.findPath(plansza11[1][1], plansza11[3][5]);
            List<Vertex> hp = Lokomotywa.findPath(plansza11[3][5], plansza11[1][1]);
        ...
        for (int i = 1; i < plansza.length - 1; i++) {
            for (int j = 1; j < plansza[i].length - 1; j++) {
                ...
                if (plansza[i][j].equals(1) && (!plansza11[i][j].equals(plansza11[a[0]][a[1]]))) {
                    ...
                    pathh = Lokomotywa.findPath(plansza11[i][j], plansza11[a[0]][a[1]]);
